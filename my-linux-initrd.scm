   ...
   (with-imported-modules (source-module-closure
     ...
     #~(begin
         ...
         (with-output-to-port (%make-void-port "w")
           (lambda ()
             (set-path-environment-variable "PATH" '("bin" "sbin" "usr/lib/klibc/bin")
                                            '#$helper-packages)))
...
loaded at boot time in the order in which they appear."
  ...
  (define my-pre-mount
    (with-imported-modules (source-module-closure
	  ...
	  (unless (file-exists? "/bin")
	    ...
	    (system* "/usr/lib/klibc/bin/ipconfig"
		   "ipconfig" "-t" "30" "-c" "any" "eth0")
	    #t))))
  ...
  ;; 	  ;; (load-foreign-library (string-append #$klibc "/lib/klibc-lBjufn1n_WjPZf2FV_qk3tdhH00"))
  ;; 	  (load-foreign-library (string-append #$klibc "/usr/lib/klibc/lib/klibc-lBjufn1n_WjPZf2FV_qk3tdhH00"))
  ;; 	  (load-foreign-library (string-append #$klibc "/usr/lib/klibc/lib/libc"))
  ;; 	  (execl
  ;; 	   (string-append
  ;; 	    #$klibc "/usr/lib/klibc/bin/ipconfig")
  ;; 	   "ipconfig" "-t" "30" "-c" "any" "eth0"))))
  ...
  (define helper-packages
    (append (file-system-packages file-systems
                                  #:volatile-root? volatile-root?)
            (if keyboard-layout
                (list loadkeys-static klibc ipxe)
                '(klibc ipxe))))
